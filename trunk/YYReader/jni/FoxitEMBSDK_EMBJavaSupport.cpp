...
//C++ version
void throwExceptionHelper(JNIEnv *env, jclass obj, const char* exceptionName,
		const char* msg) {
...
void throwException(JNIEnv *env, jclass obj, int errorcode, const char* msg) {
	switch (errorcode) {
	case FS_ERR_MEMORY:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/memoryException", msg);
		break;
	case FS_ERR_ERROR:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/errorException", msg);
		break;
	case FS_ERR_PASSWORD:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/passwordException", msg);
		break;
	case FS_ERR_FORMAT:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/formatException", msg);
		break;
	case FS_ERR_FILE:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/fileAccessException", msg);
		break;
	case FS_ERR_PARAM:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/parameterException", msg);
		break;
	case FS_ERR_STATUS:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/passwordException", msg);
	...
	case FS_ERR_TOBECONTINUED:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/toBeContinuedException", msg);
		break;
	case FS_ERR_NOTFOUND:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/searchNotFoundException", msg);
		break;
	case FS_ERR_UNPARSEDPAGE:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/unparsedPageException", msg);
		break;
	case FS_ERR_NOTEXTONPAGE:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/noTextOnPageException", msg);
		break;
	case FS_ERR_INVALID_LICENSE:
		throwExceptionHelper(env, obj,
				"com/foxitsample/exception/invalidLicenseException", msg);
	default:
		break;
	}
}
 ...
 *
 ***********************************************************************/
FS_BOOL FXMem_More(FS_LPVOID clientData, int alloc_size, void** FX_NEW_memory,
		int* FX_NEW_size) {
	LOGI("FXMem_More");
	*FX_NEW_size = alloc_size;
	if (*FX_NEW_size < 1000000)
		*FX_NEW_size = 1000000;
...
FS_RESULT FileReadBlock(void* clientData, void* buffer, FS_DWORD offset,
		FS_DWORD size) {
	FILE* file = (FILE*) clientData;
...
static FS_RESULT FileWriterBlock(FS_LPVOID clientData, const void* buffer,
		FS_DWORD offset, FS_DWORD size) {
	LOGI(
			"FileWriterBlock begin:clientData = %x buffer = %x offset = %x size = %x", clientData, buffer, offset, size);
	FILE* pFile = (FILE*) clientData;
	int ret = FALSE;
...
public:
	...
	static FPDF_PAGE GetCurrentPage(FS_LPVOID pThis, FPDF_DOCUMENT document) {
		...
		jmethodID methodID = m_pEnv->GetMethodID(clss, "FFI_GetCurrentPage",
				"(I)I");
		if (methodID == NULL)
			return NULL;

		int nCurPageHandler = m_pEnv->CallIntMethod(m_objFormFillerInfo,
				methodID, (int) document);
	...
	static FPDF_PAGE GetPage(FS_LPVOID pThis, FPDF_DOCUMENT document,
			FS_INT32 nPageIndex) {
		jclass clss = m_pEnv->GetObjectClass(m_objFormFillerInfo);
		...
		int nPageHandler = m_pEnv->CallIntMethod(m_objFormFillerInfo, methodID,
				(int) document, nPageIndex);
	...
	static void Invalidate(FS_LPVOID pThis, FPDF_PAGE page, FS_FLOAT left,
		...
		jmethodID methodID = pEnv->GetMethodID(clss, "FFI_Invalidate",
				"(IFFFF)V");
		if (methodID == NULL)
			return;

		pEnv->CallVoidMethod(m_objFormFillerInfo, methodID, (int) page, left,
				top, right, bottom);
		m_bNewThread = FALSE;
	}
	...
	static void OnSetFieldInputFocus(FS_LPVOID clientData, FPDF_FORMFIELD field,
			FS_LPCWSTR focusText, FS_DWORD nTextLen) {
		jclass clss = m_pEnv->GetObjectClass(m_objFormFillerInfo);
		jmethodID methodID = m_pEnv->GetMethodID(clss,
				"FFI_OnSetFieldInputFocus", "(ILjava/lang/String;I)V");
		if (methodID == NULL)
			return;
		...
		m_pEnv->CallVoidMethod(m_objFormFillerInfo, methodID, field, jText,
				nTextLen);

	}

	static void OutputSelectedRect(FS_LPVOID pThis, FPDF_PAGE page, double left,
	...
	static FS_INT32 SetTimer(FS_LPVOID pThis, FS_INT32 uElapse,
			FS_LPFTimerCallback lpTimerFunc) {
		jclass clss = m_pEnv->GetObjectClass(m_objFormFillerInfo);
		...
		int eventID = m_pEnv->CallIntMethod(m_objFormFillerInfo, methodID,
				uElapse, (int) lpTimerFunc);
		return eventID;
	}
...
public:
	...
	static FS_INT32 alert(FS_LPVOID clientData, FS_LPCWSTR Msg,
			FS_LPCWSTR Title, FS_INT32 Type, FS_INT32 Icon) {
		return 0;
	}
	...
	static FS_INT32 response(FS_LPVOID clientData, FS_LPCWSTR Question,
			FS_LPCWSTR Title, FS_LPCWSTR Default, FS_LPCWSTR cLabel,
			FS_BOOL bPassword, FS_LPVOID response, FS_DWORD length) {
		return 0;
	}

	static FS_INT32 getFilePath(FS_LPVOID clientData, FS_LPVOID filePath,
			FS_DWORD length) {
		return 0;
	}

	static void mail(FS_LPVOID clientData, FS_LPVOID mailData, FS_DWORD length,
			FS_BOOL bUI, FS_LPCWSTR To, FS_LPCWSTR Subject, FS_LPCWSTR CC,
			FS_LPCWSTR BCC, FS_LPCWSTR Msg) {

	}

	static void print(FS_LPVOID clientData, FS_BOOL bUI, FS_INT32 nStart,
			FS_INT32 nEnd, FS_BOOL bSilent, FS_BOOL bShrinkToFit,
			FS_BOOL bPrintAsImage, FS_BOOL bReverse, FS_BOOL bAnnotations) {

	}

	static void submitForm(FS_LPVOID clientData, FS_LPVOID formData,
			FS_DWORD length, FS_LPCWSTR URL) {

	}

	static FS_INT32 browse(FS_LPVOID clientData, FS_LPVOID filePath,
			FS_DWORD length) {
		return 1;
	}
...
public:
	...
	static void FPSI_Invalidate(void* param, FS_INT32 left, FS_INT32 top,
			FS_INT32 right, FS_INT32 bottom) {
		LOGD(
				"FPSI_Invalidate1 PSIObj: %i, env:%i, left:%i, top:%i, right%i, bottom%i", m_jPSIObj, m_pEnv, left, top, right, bottom);
		jclass clss = m_pEnv->GetObjectClass(m_jPSIObj);
		jmethodID methodID = m_pEnv->GetMethodID(clss, "FPSI_Invalidate",
				"(IIII)V");
		if (methodID == NULL) {
	...
	static FS_FLOAT FPSI_GetOpacity(void* param) {
		...
		jclass clss = m_pEnv->GetObjectClass(m_jPSIObj);
		jmethodID methodID = m_pEnv->GetMethodID(clss, "FPSI_GetOpacity",
				"()F");
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFDocLoad(JNIEnv *pEnv,
		jclass cls, jint fileRead, jstring password) {
	...
	if (!pFileRead)
		throwException(pEnv, cls, FS_ERR_PARAM,
				"FPDFDocLoad: fail to read file");

	FPDF_DOCUMENT pPDFDoc = NULL;
	FS_RESULT ret = FPDF_Doc_Load(pFileRead, lpszPassword, &pPDFDoc);
	LOGI("DocLoad = %x", ret);
	if (ret)
		throwException(pEnv, cls, ret,
				"FPDFDocLoad: Doc did not load properly");
	return (jint) pPDFDoc;
}
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFDocSaveAs(
		JNIEnv *pEnv, jclass cls, jint document, jint flag, jint pause,
		jint filewrite) {
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFDocGetPageCount(
	...
	if (count == 0) //going to fix this...
		throwException(pEnv, cls, FS_ERR_PARAM,
				"FPDFDocGetPageCount: invalid parameter. Null pointer?");
	else if (count == -1)
		throwException(pEnv, cls, FS_ERR_MEMORY,
				"FPDFDocGetPageCount: out of memory.");
...
if (ret = FPDF_Text_LoadPage((FPDF_PAGE) page, &textpage))
	throwException(pEnv, cls, ret,
			"FPDFTextLoadPage: FPDFTextLoadPage did not return success");
...
if (ret = FPDF_Text_CountChars((FPDF_TEXTPAGE) pageHandle, &count))
	throwException(pEnv, cls, ret,
			"FPDFTextCountChar: FPDFTextCountChars did not return success");
return count;
}
...
if (ret)
	throwException(pEnv, cls, ret,
			"FPDFTextCountRects: FPDFTextCountRects did not return success");
return rectCount;
...
if (ret = FPDF_Text_GetRect((FPDF_TEXTPAGE) textpage, index, &rect))
	throwException(pEnv, cls, ret,
			"FPDFTextGetRect: FPDFTextGetRect did not return success");

jclass cls_r = pEnv->FindClass(
		(const char*) "FoxitEMBSDK/EMBJavaSupport$RectangleF");
...
FS_DWORD bufflen = 0;
FS_RESULT ret;
if (ret = FPDF_Text_GetText((FPDF_TEXTPAGE) textpage, nStart, nCount, NULL,
		&bufflen))
	throwException(pEnv, cls, ret,
			"FPDFTextGetText: FPDFTextGetText did not return success");
FS_WCHAR* pBuff = new FS_WCHAR[bufflen + 1];
pBuff[bufflen] = 0;
if (ret = FPDF_Text_GetText((FPDF_TEXTPAGE) textpage, nStart, nCount, pBuff,
		&bufflen))
	throwException(pEnv, cls, ret,
			"FPDFTextGetText: FPDFTextGetText did not return success");
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFTextFindStart(
	JNIEnv *pEnv, jclass cls, jint textpage, jstring findwhat, jlong flag,
	jint startindex) {
...
FS_RESULT ret = FPDF_Text_FindStart(pTextPage, (FS_WCHAR*) wcFind, flag,
		startindex, &searchHandle);
if (ret != 0)
	throwException(pEnv, cls, ret,
			"FPDFTextFindStart: FPDFTextFindStart did not return success");
...
if (ret != 0)
	throwException(pEnv, cls, ret,
			"FPDFTextFindNext: FPDFTextFindNext did not return success");
return isMatch;
...
if (ret != 0)
	throwException(pEnv, cls, ret,
			"FPDFTextFindPrev: FPDFTextFindPrev did not return success");
return isMatch;
}
...
if (ret != 0)
	throwException(pEnv, cls, ret,
			"FPDFTextGetSchResultIndex: FPDFTextGetSchResultIndex did not return success");
return index;
}
...
if (ret != 0)
	throwException(pEnv, cls, ret,
			"FPDFTextGetSchCount: FPDFTextGetSchCount did not return success");
return count;
}
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFPagePageToDeviceRect(
	JNIEnv *pEnv, jclass clss, jint page, jint start_x, jint start_y,
	jint size_x, jint size_y, jint rotate, jobject rect) {
if (page == 0)
	throwException(pEnv, clss, FS_ERR_PARAM,
			"FPDFPagePageToDeviceRect: Page is NULL");
FPDF_PAGE pPage = (FPDF_PAGE) page;

if (!rect)
	throwException(pEnv, clss, FS_ERR_PARAM,
			"FPDFPagePageToDeviceRect: Rect is NULL");
FS_RECT deviceRect;
...
FPDF_Page_PageToDeviceRect(pPage, start_x, start_y, size_x, size_y, rotate,
		&deviceRect);
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFRenderPageStart(
	JNIEnv *pEnv, jclass cls, jint dib, jint page, jint start_x, jint start_y,
	jint size_x, jint size_y, jint rotate, jint flags, jobject clip,
	jint pauseHandler) {
FPDF_PAGE pPDFPage = (FPDF_PAGE) page;
if (!pPDFPage)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FPDFRenderPageStart: null page pointer");

FS_BITMAP bitmap = (FS_BITMAP) dib;
if (!bitmap)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FPDFRenderPageStart: null dib pointer");
FS_PAUSE* pPause = (FS_PAUSE*) pauseHandler;
...
if (clip != NULL) {
	...
	FPDF_RenderPage_Start(bitmap, pPDFPage, start_x, start_y, size_x, size_y,
			rotate, flags, &clipRect, pPause);
} else {
	ret = FPDF_RenderPage_Start(bitmap, pPDFPage, start_x, start_y, size_x,
			size_y, rotate, flags, NULL, pPause);
...
if (!pPDFPage)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FPDFRenderPageContinue: null page pointer");
...
if (page == 0)
	throwException(pEnv, clss, FS_ERR_PARAM,
			"FPDFPageDeviceToPageRect: null page pointer");
FPDF_PAGE pPage = (FPDF_PAGE) page;

if (!rect)
	throwException(pEnv, clss, FS_ERR_PARAM,
			"FPDFPageDeviceToPageRect: invalid rect object");
FS_RECT deviceRect;
jclass cls = pEnv->GetObjectClass(rect);
jfieldID idA = pEnv->GetFieldID(cls, "left", "I");
jfieldID idB = pEnv->GetFieldID(cls, "top", "I");
...
FPDF_Page_DeviceToPageRect(pPage, start_x, start_y, size_x, size_y, rotate,
		&deviceRect);
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFPageDeviceToPageRectF(
	JNIEnv *pEnv, jclass cls, jint page, jint start_x, jint start_y,
	jint size_x, jint size_y, jint rotate, jobject rectf) {
if (page == 0)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FPDFPageDeviceToPageRectF: null page pointer");
FPDF_PAGE pPage = (FPDF_PAGE) page;

if (!rectf)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FPDFPageDeviceToPageRectF: invalid rect object");
FS_RECTF deviceRect;
...
FPDF_Page_DeviceToPageRectF(pPage, start_x, start_y, size_x, size_y, rotate,
		&deviceRect);
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FSBitmapCreate(
	JNIEnv *pEnv, jclass cls, jint width, jint height, jint format,
	jbyteArray buffer, jint stride) {
FS_BITMAP bitmap = NULL;
void* pBuffer = NULL;
int nRet;
if (buffer != NULL) {
	unsigned char* pBuf = (unsigned char*) (pEnv->GetByteArrayElements(buffer,
			NULL));
	pBuffer = (void*) pBuf;
}
...
if (nRet)
	throwException(pEnv, cls, nRet,
			"FSBitmapCreate: FS_Bitmap_Create did not return success");
return (jint) bitmap;
}
...
if (!bitmap || stride < 0 || height < 0 || buffer == NULL)
	throwException(pEnv, cls, FS_ERR_PARAM,
			"FSBitmapGetBuffer: null or invalid dib pointer");
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_FORMFILL_INFO),
		(FS_LPVOID*) &pFormFillerInfo);
if (nRet)
...
pFormFillerInfo->OnSetFieldInputFocus =
		CPDF_FormFillerInfo::OnSetFieldInputFocus;
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_FORMFILL_JSPLATFORM),
		(FS_LPVOID*) &pJsPlatform);
if (nRet)
...
FS_RESULT ret1 = FPDF_FormFill_InitEnvironment(pPDFDoc, pFormFillerInfo,
		&formHandler);
  ...
  (JNIEnv *pEnv, jclass cls, jint nFormHandler,jint page )
{
	...
	LOGI(
	"Java_FoxitEMBSDK_EMBJavaSupport_FPDF_1FormFill_1OnAfterLoadPage RET IS  %x", ret);
}
...
JNIEnv *pEnv, jclass cls, jint nFormHandler, jint page, jint evenflag,
...
FS_BOOL bRet = FPDF_FormFill_OnMouseMove(formHandler, pPage, evenflag, page_x,
	page_y);
return (jboolean) bRet;
}
...
FS_BOOL bRet = FPDF_FormFill_OnLButtonUp(formHandler, pPage, evenflag, page_x,
	page_y);
return (jboolean) bRet;
}
...
FS_BOOL bRet = FPDF_FormFill_OnLButtonDown(formHandler, pPage, evenflag, page_x,
	page_y);
return (jboolean) bRet;
}

JNIEXPORT jboolean JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFFormFillOnSetText(
JNIEnv *pEnv, jclass cls, jint nFormHandler, jint page, jstring text,
jint evenflag) {
if (nFormHandler == 0 || page == 0)
return (jboolean) FALSE;
FPDF_FORMENV formHandler = (FPDF_FORMENV) nFormHandler;
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(const unsigned short) * nLen,
	(FS_LPVOID*) &pText);
if (nRet)
throwException(pEnv, cls, nRet, "FPDFFormFillOnSetText: out of memory");

memset((void*) pText, '\0', sizeof(const unsigned short) * nLen);
memcpy((void*) pText, pTempEnv->GetStringChars(text, NULL),
	nLen * sizeof(const unsigned short));
pTempEnv->ReleaseStringChars(text, pText);
FS_BOOL bRet = FPDF_FormFill_OnSetText(formHandler, pPage, pText, nLen,
	evenflag);
return (jboolean) bRet;
}
...
JNIEnv *pEnv, jclass clss, jstring author, jlong color, jint opacity,
jobject rect, jstring content, jint page) {
FPDF_PAGE pPage = (FPDF_PAGE) page;
FPDF_ANNOT_NOTEINFO* pNoteInfo = NULL;
FS_RESULT ret;

FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_NOTEINFO),
	(FS_LPVOID*) &pNoteInfo);
if (nRet)
...
memcpy(pNoteInfo->author, pEnv->GetStringChars(author, NULL),
	nLen * sizeof(FS_WCHAR));
...
nRet = FS_Memory_Alloc(sizeof(FS_WCHAR) * (nLen + 1),
	(FS_LPVOID*) (&(pNoteInfo->contents)));
if (nRet)
throwException(pEnv, clss, nRet,
		"FPDFNoteInfoAlloc: note info content out of memory");

if (!pNoteInfo->contents)
return (jint) NULL;
memset((void*) pNoteInfo->contents, '\0', (nLen + 1) * sizeof(FS_WCHAR));

memcpy((void*) pNoteInfo->contents, pEnv->GetStringChars(content, NULL),
	nLen * sizeof(FS_WCHAR));
...
JNIEnv *pEnv, jclass cls, jstring author, jlong color, jint opacity,
jboolean busebezier, jboolean boptimize, jint line_width, jint line_count) {
FPDF_ANNOT_PENCILINFO* pPencilInfo = NULL;
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_PENCILINFO),
	(FS_LPVOID*) &pPencilInfo);
if (nRet)
...
memcpy(pPencilInfo->author, pEnv->GetStringChars(author, NULL),
	nLen * sizeof(FS_WCHAR));
...
FPDF_ANNOT_LINE* pLineInfoArray = (FPDF_ANNOT_LINE*) malloc(
	sizeof(FPDF_ANNOT_LINE) * line_count);
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_HIGHLIGHTINFO),
	(FS_LPVOID*) &pHighlightInfo);
if (nRet)
...
memcpy(pHighlightInfo->author, pEnv->GetStringChars(author, NULL),
	nLen * sizeof(FS_WCHAR));
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_QUAD) * quad_count,
	(FS_LPVOID*) &pQuadsInfo);
if (nRet)
...
JNIEnv *pEnv, jclass clss, jstring author, jlong color, jint opacity,
jobject rect, jstring subject, jstring image_path) {
FPDF_ANNOT_STAMPINFO* pStampInfo = NULL;
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_STAMPINFO),
	(FS_LPVOID*) &pStampInfo);
if (nRet)
throwException(pEnv, clss, nRet,
		"FPDFStampInfoAlloc: stamp info out of memory");
...
memcpy(pStampInfo->author, pEnv->GetStringChars(author, NULL),
	nLen * sizeof(FS_WCHAR));
...
memcpy(pStampInfo->name, pEnv->GetStringChars(subject, NULL),
	nLen * sizeof(FS_WCHAR));
...
if (nRet)
throwException(pEnv, clss, nRet,
		"FPDFStampInfoAlloc: file buffer out of memory");
...
JNIEnv *pEnv, jclass clss, jstring author, jlong color, jint opacity,
jobject rect, jstring filepath) {
FPDF_ANNOT_FILEATTACHMENTINFO* pFileAttachmentInfo = NULL;
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_ANNOT_FILEATTACHMENTINFO),
	(FS_LPVOID*) &pFileAttachmentInfo);
if (nRet)
throwException(pEnv, clss, nRet,
		"FPDFFileAttachmentInfoAlloc: file attachment info out of memory");
...
memcpy(pFileAttachmentInfo->author, pEnv->GetStringChars(author, NULL),
	nLen * sizeof(FS_WCHAR));
...
if (nRet)
throwException(pEnv, clss, nRet,
		"FPDFFileAttachmentInfoAlloc: file attachment buffer out of memory");
...
ret = FPDF_Annot_Add(pPage, annot_type, (void*) data, size,
	(FPDF_ANNOT*) &nIndex);
...
JNIEnv *pEnv, jclass cls, jint reflowPage, jint page, jint width,
jint fitPageHight, jint pauseHandler, jint nFlag) {
FS_PAUSE* pPause = (FS_PAUSE*) pauseHandler;
FS_RESULT ret = FPDF_Reflow_StartParse((FPDF_REFLOWPAGE) reflowPage,
	(FPDF_PAGE) page, width, fitPageHight, pPause, nFlag);
return ret;
}
...
JNIEnv *pEnv, jclass cls, jint dib, jint reflowPage, jint start_x, jint start_y,
jint size_x, jint size_y, jint rotate, jint pauseHandler) {
FS_BITMAP bitmap = (FS_BITMAP) dib;
FS_PAUSE* pPause = (FS_PAUSE*) pauseHandler;
FS_RESULT ret = FPDF_Reflow_StartRender(bitmap, (FPDF_REFLOWPAGE) reflowPage,
	start_x, start_y, size_x, size_y, rotate, pPause);
return ret;
}
...
FS_RESULT ret = FPDF_Reflow_SetDitherBits((FPDF_REFLOWPAGE) reflowPage,
	DitherBits);
return ret;
}
...
FS_BOOL MyMapFont(const char* name, int charset, unsigned int flags, int weight,
...
FS_BOOL MyMapFont(FS_LPVOID mapper, FS_LPCSTR name, FS_INT32 charset,
FS_DWORD flags, FS_INT32 weight, FS_CHAR* path, FS_INT32* face_index) {
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPSI_APPCALLBACK),
(FS_LPVOID*) &pCallback);
if (nRet)
throwException(pEnv, cls, nRet, "FPSIInitAppCallback: out of memory");
...
JNIEnv *pEnv, jclass cls, jint nPSIHandle, jfloat x, jfloat y, jfloat pressure,
jint flag) {
CPDF_PSI::SetEnv(pEnv);
if (nPSIHandle == 0)
return (jint) FS_ERR_PARAM;
...
JNIEXPORT jint JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPSIRender(JNIEnv *pEnv,
jclass cls, jint nPSIHandle, jint dib, jint left, jint top, jint right,
jint bottom, jint src_left, jint src_top) {
if (nPSIHandle == 0)
return (jint) FS_ERR_PARAM;

FPSI_HANDLE hHandle = (FPSI_HANDLE) nPSIHandle;
FS_BITMAP bitmap = (FS_BITMAP) dib;
return (jint) FPSI_Render(hHandle, bitmap, left, top, right, bottom, src_left,
src_top);
}
...
FS_BOOL FilterInput(FS_LPVOID filter, FPDF_DECRYPT_OUTPUT output,
FS_LPCBYTE src_data, FS_DWORD src_len) {
...
FS_BOOL GetCryptInfo(FS_LPVOID handler, FS_INT32* cipher, FS_LPBYTE buffer,
FS_LPDWORD keylen) {
return false;
}
...
FS_DWORD EncryptGetSize(FS_LPVOID handler, FS_INT32 objnum, FS_INT32 version,
FS_LPCBYTE src_buf, FS_DWORD src_size) {

return src_size;
}

FS_BOOL EncryptContent(void* handler, int objnum, int version,
FS_LPCBYTE src_buf, FS_DWORD src_size, FS_LPBYTE dest_buf,
FS_DWORD* dest_size) {
	//encrypt data
...
FS_BOOL MyGetPubKeyVerify(const char* envelop, FS_INT32 len,
unsigned char* seed_buf, FS_DWORD* nlen);
};

FS_BOOL CPDFPKISecurityHandle::MyGetPubKeyVerify(const char* envelop,
FS_INT32 len, unsigned char* seed_buf, FS_DWORD* nlen) {
	//this ,customer must use their way to get the publick key.
return TRUE;
}

FS_BOOL MyGetPubKeyVerify(FS_LPVOID pThis, FS_LPCSTR envelop, FS_DWORD len,
FS_LPBYTE seed_buf, FS_LPDWORD nlen) {
...
JNIEnv *pEnv, jclass cls, jint pDoc, jstring handler_name, jint pHandler,
jstring filepath) {
...
FS_RESULT nRet = FPDF_Security_CustomEncrypt(pPDFDoc, handlername,
pSecurityHandler, &fileWrite);
fclose(file);
return nRet;
}
...
JNIEnv *pEnv, jclass cls, jint envelopes, jbyteArray envelopeData,
jint dataLen) {
FPDF_ENVELOPES pEnvelopes = (FPDF_ENVELOPES) envelopes;
FS_LPVOID lpenvelopeData = (FS_LPVOID) pEnv->GetByteArrayElements(envelopeData,
NULL);
...
JNIEnv *pEnv, jclass cls, jint pDoc, jint nCipher, jboolean bEncryptMetaData,
jint envelops, jbyteArray key, jint nKeyBytes, jstring filepath) {
FPDF_DOCUMENT pPDFDoc = (FPDF_DOCUMENT) pDoc;
...
FS_RESULT nRet = FPDF_Security_CertEncrypt(pPDFDoc, nCipher, bEncryptMetaData,
pEnvelops, lpKey, nKeyBytes, &fileWrite);
fclose(file);
return nRet;
}
...
file_path = pEnv->GetStringUTFChars(filepath, NULL);
LOGI(
"Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: file_path = %s", file_path);
void* file = fopen(file_path, "wb");
LOGI("Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: file = %x", file);

void* fileTest = fopen("/data/data/com.foxitsample.annotations/FoxitTest.txt",
"wb+");
LOGI(
"Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: fileTest = %x", fileTest);
...
FS_RESULT gSignDocument(FS_LPVOID clientData, FPDF_SIGNATURE_FIELD* sigField,
FS_LPBYTE srcbuffer, FS_DWORD dwSize, FS_LPBYTE* signedbuffer,
FS_DWORD* dwSignedSize) {
...
FS_RESULT gVerifySignature(FS_LPVOID clientData, FPDF_SIGNATURE_FIELD* sigField,
FS_LPBYTE srcbuffer, FS_DWORD dwSize, FS_LPBYTE signedbuffer,
FS_DWORD dwSignedSize, int* state) {
...
FS_RESULT nRet = FS_Memory_Alloc(sizeof(FPDF_SIGNATURE_HANDLER),
(FS_LPVOID*) &pSignatureHandler);
if (!pSignatureHandler)
throwException(pEnv, cls, FS_ERR_MEMORY,
"FPDFSignatureAllocHandler: out of memory");
...
if (nRet != FS_ERR_SUCCESS) {
throwException(pEnv, cls, nRet,
"FPDFSignatureCount:  signature did not get count properly");
}
return nCount;
}
...
if (nRet != FS_ERR_SUCCESS) {
throwException(pEnv, cls, nRet,
"FPDFSignatureGetByIndex:  signature did not get Signature properly");
}
...
if (nRet != FS_ERR_SUCCESS) {
throwException(pEnv, cls, nRet,
"FPDFSignatureVerify:  signature did not verify properly");
return FALSE;
}
